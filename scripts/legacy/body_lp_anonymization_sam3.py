...
def anonymize_body_image(image: np.ndarray, bodies: list[Body], anon_function) -> np.ndarray:
    image = image.copy()
    
    for body in bodies:
        if body.body_mask is None:
            continue
        ...
        if x_max > x_min and y_max > y_min:
            ...
            image[y_min:y_max, x_min:x_max] = anon_roi
...
def process_image(args):
    image_path, output_dir, anon_methods, body_detector, lp_detector = args
    ...
    if not bodies and not license_plates:
        logger.info(f"No bodies or license plates detected in {image_path.name}")
    
    for method_pair in anon_methods:
        if isinstance(method_pair, tuple):
            body_method, lp_method = method_pair
        else:
            body_method = lp_method = method_pair
        
        output_path = output_dir / f"{stem}_anon_{body_method}_{lp_method}.png"
        if output_path.exists():
            continue
        
        body_anon_func = ANON_FUNCTIONS.get(body_method, anonymize_white)
        lp_anon_func = ANON_FUNCTIONS.get(lp_method, anonymize_white)
        
        anon_image = anonymize_body_image(image.copy(), bodies, body_anon_func)
        anon_image = anonymize_lp_image(anon_image, license_plates, lp_anon_func)
...
def main():
    parser = argparse.ArgumentParser(description="Anonymize bodies and license plates using SAM3")
    parser.add_argument("--image_dir", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--body_anon", type=str, default="white", 
                       help="body anonymization: white, gauss, pixel, lda or all")
    parser.add_argument("--lp_anon", type=str, default="white", 
                       help="license plate anonymization: white, gauss, pixel or all")
    ...
    def parse_methods(method_str):
        if method_str == "all":
            return ["white", "gauss", "pixel"]
        return [method_str]
    
    body_methods = parse_methods(args.body_anon)
    lp_methods = parse_methods(args.lp_anon)
    
    # All combinations of body and lp methods
    anon_methods = []
    for bm in body_methods:
        for lm in lp_methods:
            anon_methods.append((bm, lm))
    
    completed, completed_set = check_progress(output_dir, [m[0] for m in anon_methods], len(images))
    ...
    print(f"Processing {len(images)} images with SAM3")
    print(f"Anonymization methods: {anon_methods}")

    body_detector = SAM3BodyDetector(threshold=args.body_threshold)
    lp_detector = SAM3LicensePlateDetector(threshold=args.lp_threshold)

    tasks = [(img, output_dir, anon_methods, body_detector, lp_detector) for img in images]
