...
def process_image(args):
    image_path, output_dir, anon_methods, body_detector = args
    ...
    if not bodies:
        logger.info(f"No bodies detected in {image_path.name}")
    ...
    for method in anon_methods:
        ...
        anon_image = anonymize_body_image(image.copy(), bodies, anon_function)
...
def main():
    parser = argparse.ArgumentParser(description="Anonymize bodies using SAM3")
    parser.add_argument("--image_dir", type=str, required=True)
    parser.add_argument("--output_dir", type=str, required=True)
    parser.add_argument("--anon_function", type=str, default="white", 
                       help="comma-separated for multiple: white,gauss,pixel or 'all'")
    ...
    if args.anon_function == "all":
        anon_methods = ["white", "gauss", "pixel"]
    else:
    ...
    print(f"Processing {len(images)} images with SAM3 body detector")
    print(f"Anonymization methods: {anon_methods}")

    body_detector = SAM3BodyDetector(threshold=args.body_threshold)

    tasks = [(img, output_dir, anon_methods, body_detector) for img in images]
